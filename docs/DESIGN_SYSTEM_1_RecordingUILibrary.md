...
Минималистичная, premium UI библиотека для интерфейса записи микрофона и обработки аудио. Включает:

- **Визуализация волны** (real-time амплитуда, 30-bar ring buffer)
- **Таймер** (MM:SS формат, monospaced шрифт)
- **Статус индикатор** (IDLE/RECORDING/PROCESSING/COMPLETE/ERROR с пульсом)
- **Индикатор загрузки** (при обработке, animated progress bar)
