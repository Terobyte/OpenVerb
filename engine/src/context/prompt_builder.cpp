...
// MVP4+ i18n note: spec requires en/ru/es/fr/de/ja templates;
//   when adding localization, move SYSTEM_PROMPT and style strings to
//   templates/ files keyed by locale and load at runtime.
// ---------------------------------------------------------------------------
...
// Must match spec (docs/superpowers/specs/2026-04-13-openverb-architecture-design.md lines 216-232).
// MVP4+ i18n: replace with per-locale template loaded from templates/.
...
// ---------------------------------------------------------------------------
// MVP4+ i18n: localize style descriptions per locale template file.
...
std::pair<std::string, std::string> build_prompt(const PromptContext& ctx) {
    ...
    return {xml, "Output ONLY the final text:"};
}
